...
class ControladorConfiguracion(QObject):
    ...
    def _normalizar_lista_textos(self, valor: Any) -> list[str]:
        """
        Normaliza listas recibidas desde QML.

        Acepta:
        - list / tuple
        - string separado por "/"

        Devuelve una lista limpia, sin valores vacíos ni duplicados exactos.
        """
        elementos: list[str] = []

        if isinstance(valor, (list, tuple)):
            elementos = [str(item).strip() for item in valor if str(item).strip()]
        else:
            texto = str(valor).strip()

            if texto:
                elementos = [parte.strip() for parte in texto.split("/") if parte.strip()]
    ...
    def guardarConfiguracionSede(self, datos: dict[str, Any]) -> str:
        try:
            ...
            configuracion_actual["coordinadora"] = str(datos.get("coordinadora", "")).strip()
            configuracion_actual["profesionalesIntervinientes"] = self._normalizar_lista_textos(
    ...
    def cargarConfiguracionSede(self) -> dict[str, Any]:
        try:
            ...
            return {
                ...
                "profesionalesIntervinientes": self._normalizar_lista_textos(
