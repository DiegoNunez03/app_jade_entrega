...
ApplicationWindow {
    ...
    NavBar {
        ...
        onOpcionSeleccionada: function(opcion) {
            ...
            if (opcion === 3) {
                mainWindow.sectionActive = 2
                sectiontwo.abrirAltaTutorDesdeMenu()
                return
            }

            if (opcion === 4) {
                mainWindow.sectionActive = 4
                sectionfour.abrirBajaDesdeMenu()
                return
            }
    ...
    SectionOne {
        ...
        onBajaSolicitada: {
            console.log("MAIN → navegando a sección de bajas")
            mainWindow.sectionActive = 4
    ...
    SectionFour {
        ...
        visible: mainWindow.sectionActive === 4

        fechaAutomatica: mainWindow.fechaAutomaticaGlobal
        fechaActual: mainWindow.fechaActualGlobal
    }
    ...
    SectionFive {
        ...
        fechaAutomaticaActual: mainWindow.fechaAutomaticaGlobal
        fechaActualConfigurada: mainWindow.fechaActualGlobal
        ...
        onFechaAutomaticaCambiada: function(valor) {
            mainWindow.fechaAutomaticaGlobal = valor

            // Logger.linea() || NO BORRAR
            // Logger.simple("MAIN", "fecha automática global actualizada", mainWindow.fechaAutomaticaGlobal) || NO BORRAR
            // Logger.linea() || NO BORRAR
        }

        onFechaActualCambiada: function(valor) {
            mainWindow.fechaActualGlobal = valor

            // Logger.linea() || NO BORRAR
            // Logger.simple("MAIN", "fecha actual global actualizada", mainWindow.fechaActualGlobal) || NO BORRAR
            // Logger.linea() || NO BORRAR
        }
    }
